...
const { requestFailureMessage } = require('./request-error');

const http = new Request();

let shroomPlatform = 'H5';
// #ifdef APP-PLUS
shroomPlatform = 'APP';
// #endif
// #ifdef MP-WEIXIN
shroomPlatform = 'MP_WEIXIN';
// #endif
...
http.setConfig(config => {
	...
	config.header = {
		...config.header,
		...systemInfoHeaders,
		'x-shroom-platform': shroomPlatform
	};
	return config;
});
...
http.interceptor.request(
	...
	error => {
		...
		if (message) mHelper.toast(message);
