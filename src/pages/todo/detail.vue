	...
	<view class="detail-page">
		...
		<scroll-view v-if="task" class="content" scroll-y>
			...
			<view class="actions">
				...
				<button v-if="task.status === 'in_progress'" class="primary" @tap="showResultSheet = true">记录结果并完成</button>
				<button v-if="task.status === 'pending'" @tap="showResultSheet = true">标记完成</button>
			...
			<view v-if="task.status === 'completed' || task.result" class="section result">
		...
		<view v-if="showResultSheet" class="mask" @tap="showResultSheet = false"><view class="sheet" @tap.stop><view class="handle"></view><text class="sheet-title">实际发生了什么？</text><textarea v-model="resultText" maxlength="5000" auto-height placeholder="结果可以以后再补，不写也能完成" /><view class="result-note"><text>系统会留下独立行动记录</text><text>不会伪造或修改你的日记正文。</text></view><button class="confirm" :disabled="saving" @tap="complete">{{ saving ? '保存中…' : '确认完成' }}</button></view></view>
	</view>
</template>

<script>
import { todoDetail, todoStatus } from '@/api/todo';
export default {
	data() { return { statusBarHeight: 0, taskId: '', task: null, timeZone: this.localTimeZone(), showResultSheet: false, resultText: '', saving: false }; },
	...
	methods: {
		...
		async load() { try { const res = await this.$http.get(todoDetail, { id: this.taskId, timeZone: this.timeZone }); if (res.code !== 200) throw new Error(res.message); this.task = res.data; this.resultText = res.data.result || ''; } catch (e) { uni.showToast({ title: e.message || '待办加载失败', icon: 'none' }); } },
		...
		async complete() { if (this.saving) return; this.saving = true; try { const res = await this.$http.patch(todoStatus, { id: this.task.id, action: 'COMPLETE', version: this.task.version, result: this.resultText, operationId: this.requestId(), timeZone: this.timeZone }); if (res.code !== 200) throw new Error(res.message); this.task = { ...this.task, ...res.data }; this.showResultSheet = false; uni.showToast({ title: '已完成并留下行动记录', icon: 'none' }); this.load(); } catch (e) { uni.showToast({ title: e.message || '完成失败', icon: 'none' }); } finally { this.saving = false; } },
		...
		eventLabel(type) { return { CREATED: '创建待办', UPDATED: '调整待办', STARTED: '开始推进', COMPLETED: '完成行动', RESTORED: '恢复为未完成', CANCELLED: '取消待办', SKIPPED: '跳过本次', DIARY_LINKED: '关联日记' }[type] || type; },
