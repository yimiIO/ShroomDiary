	...
	<view class="edit-page">
		...
		<scroll-view class="form" scroll-y>
			...
			<view class="settings">
				...
				<view class="setting date"><text>安排日期</text><picker mode="date" :value="form.scheduledDate || today" @change="changeScheduledDate"><text>{{ form.scheduledDate || '未设置' }}</text></picker><button v-if="form.scheduledDate && !repeatEnabled" @tap="form.scheduledDate = ''">清除</button></view>
				<view class="setting date"><text>开始时间</text><picker mode="time" :value="form.scheduledStartTime || '09:00'" @change="form.scheduledStartTime = $event.detail.value"><text>{{ form.scheduledStartTime || '未设置' }}</text></picker><button v-if="form.scheduledStartTime" @tap="clearScheduleTime">清除</button></view>
				<view class="setting date"><text>结束时间</text><picker mode="time" :value="form.scheduledEndTime || form.scheduledStartTime || '09:30'" @change="form.scheduledEndTime = $event.detail.value"><text>{{ form.scheduledEndTime || '未设置' }}</text></picker><button v-if="form.scheduledEndTime" @tap="form.scheduledEndTime = ''">清除</button></view>
				...
				<view class="setting repeat-toggle"><view><text>设为习惯</text><text>提前生成固定计划</text></view><switch color="#52643d" :checked="repeatEnabled" :disabled="!!(task && task.recurrence)" @change="toggleRepeatEnabled" /></view>
				<view v-if="repeatEnabled" class="repeat-editor">
					<template v-if="task && task.recurrence"><text>修改范围</text><view class="scope-buttons"><button :class="{ active: repeatScope === 'INSTANCE' }" @tap="repeatScope = 'INSTANCE'">仅本次</button><button :class="{ active: repeatScope === 'FUTURE' }" @tap="repeatScope = 'FUTURE'">本次及以后</button></view><text class="scope-note">{{ repeatScope === 'INSTANCE' ? '修改当前这一次，不影响其他日期。' : '过去已完成的记录保留，从这一次开始使用新安排。' }}</text></template>
					<view v-if="!task || !task.recurrence || repeatScope === 'FUTURE'" class="repeat-fields">
						...
						<view class="setting"><text>{{ task && task.recurrence ? '新安排从' : '开始日期' }}</text><picker mode="date" :value="recurrence.startsOn" @change="changeRepeatStartDate"><text>{{ recurrence.startsOn }}</text></picker></view>
...
export default {
	...
	methods: {
		...
		clearScheduleTime() { this.form.scheduledStartTime = ''; this.form.scheduledEndTime = ''; },
