...
export default {
	...
	computed: {
		...
		paymentReady() {
			...
			if (this.h5InsideWechat()) return false;
			return Boolean(this.paymentChannel.live);
		},
		paymentUnavailableReason() {
			if (this.h5InsideWechat()) return '微信内网页不能调起 H5 支付，请在系统浏览器打开。';
	...
	methods: {
		...
		async recharge() {
			...
			try {
				await this.acceptAgreements();
				const clientPlatform = this.paymentPlatform();
				const idempotencyKey = `recharge-${Date.now()}-${Math.random().toString(36).slice(2)}`;
				const response = await this.$http.post(billingPaymentOrders, {
					amountYuan: amount,
					clientPlatform,
					idempotencyKey
				});
				...
				// #ifdef H5
				const returnUrl = `${window.location.origin}/pages/shroom/wallet`;
				window.location.href = `${response.data.payment.h5Url}&redirect_url=${encodeURIComponent(returnUrl)}`;
				// #endif
			} catch (error) {
