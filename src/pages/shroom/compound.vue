		...
		<view class="page-shell">
			...
			<template v-else>
				...
				<view v-if="showPlanEditor" class="editor-panel plan-editor">
					...
					<view class="editor-intro"><text>只需要回答 4 件事</text><text>复利原理、回报方式和基础衡量由系统根据你选的类型设置，不需要你重复解释。</text></view>
					...
					<view class="validation-notice"><text>先运行，再判断</text><text>创建后只是“验证中”。系统不会因为你填完表单，就宣布复利已经成立。</text></view>
				...
				<template v-if="home.plans.length">
					...
					<view class="validation-row"><text :class="{ validating: plan.validation.status === 'VALIDATING', compounding: plan.validation.status === 'COMPOUNDING', linear: plan.validation.status === 'LINEAR', protection: plan.validation.status === 'PROTECTION' }">{{ validationLabel(plan.validation.status, plan) }}</text><text>{{ plan.investmentKind === 'PROTECTION' ? '保障型' : '增长型' }}</text><text v-if="plan.validation.dueAt">首次判断 {{ compactDate(plan.validation.dueAt) }}</text><button @tap="openValidation(plan)">更新判断</button></view>
						...
						<view v-if="plan.archetypeKey === 'financial_capital'" class="finance-boundary compact"><text>边界持续生效</text><text>这里只记录你的计划和已发生事实，不代表未来收益，也不会给出具体产品或交易建议。</text></view>
...
export default {
	...
	computed: {
		...
		canSavePlan() { const d = this.planDraft; const boundaryReady = !this.isFinancialPlanDraft || Boolean(this.editingPlanId) || d.financialBoundaryAccepted; return Boolean((this.editingPlanId || d.archetypeKey) && d.title.trim() && d.principalDefinition.trim() && d.desiredOutcome.trim() && d.currentStep.trim() && boundaryReady); },
		validationGuidance() {
