		...
		<view class="page-shell">
			...
			<template v-else>
				...
				<view v-if="showPlanEditor" class="editor-panel plan-editor">
					...
					<view class="editor-intro"><text>{{ isFinancialPlanDraft ? '先定义计划，再逐步核账' : '只需要回答 4 件事' }}</text><text>{{ isFinancialPlanDraft ? '这里只确定名称、范围、长期目的和第一项核对行动；精确金额进入私人加密台账后再填写。' : '复利原理、回报方式和基础衡量由系统根据你选的类型设置，不需要你重复解释。' }}</text></view>
				...
				<template v-if="home.plans.length">
					...
					<view v-for="(plan, index) in home.plans" :key="plan.id" class="plan-card" :class="{ primary: index === 0, protection: plan.investmentKind === 'PROTECTION' }">
						<view class="plan-head"><view><text>{{ plan.archetype ? plan.archetype.name : '历史计划' }}</text><text>{{ plan.title }}</text></view><button @tap="plan.archetypeKey === 'financial_capital' ? openFinancialLedger(plan) : openEditPlan(plan)">{{ plan.archetypeKey === 'financial_capital' ? '打开' : '编辑' }}</button></view>
...
export default {
	...
	computed: {
		...
		canSavePlan() { const d = this.planDraft; const boundaryReady = !this.isFinancialPlanDraft || Boolean(this.editingPlanId) || d.financialBoundaryAccepted; const principalReady = this.isFinancialPlanDraft || d.principalDefinition.trim(); return Boolean((this.editingPlanId || d.archetypeKey) && d.title.trim() && principalReady && d.desiredOutcome.trim() && d.currentStep.trim() && boundaryReady); },
		validationGuidance() {
	...
	methods: {
		...
		closeEditors() { this.showPlanEditor = false; this.showAdvancedPlan = false; this.showWeekEditor = false; this.showProgressEditor = false; this.showValidationEditor = false; this.showBlockerEditor = false; this.resultDraft = null; this.blockerInsight = null; this.diaryReviewDraft = null; },
