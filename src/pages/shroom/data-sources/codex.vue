	...
	<view class="page">
		...
		<view class="shell">
			<view class="header">
				...
				<view><text class="kicker">CODEX CONNECTION</text><text class="title">Codex</text><text class="subtitle">把 Codex 中已经发生的最小任务事实连接到菇日记。</text></view>
			</view>
			...
			<view class="source-card">
				...
				<view v-else class="connection-body">
					...
					<view v-if="connection.status === 'PENDING' && !pairing" class="pending-missing"><text>上次连接指令已不可再显示。</text><button @tap="createConnection">重新生成连接指令</button></view>

					<view class="pairing" v-if="pairing">
						<text class="pairing-label">连接指令 10 分钟内有效</text>
						<text class="pairing-help">复制后粘贴给这台电脑上的 Codex，它会自动安装连接器并完成配对。</text>
						<view class="command-row"><text>{{ pairing.command }}</text><button :class="{ copied: copiedTarget === 'command' }" @tap="copy(pairing.command, 'command')">{{ copiedTarget === 'command' ? '已复制' : '复制给 Codex' }}</button></view>
					</view>
					...
					<view class="settings" v-if="connection.status === 'ACTIVE' || connection.status === 'PAUSED'">
						...
						<view class="interval-setting"><view><text>每天 19:00 自动同步</text><text>如果当时关机或休眠，本机恢复运行后会补拉；增量同步不会重复保存任务。</text></view></view>
					</view>
...
export default {
	data() {
		return {
			...
			copyTimer: null,
			copiedTarget: null,
			intervals: [{ value: 24, label: '每天 19:00' }]
		};
	},
	computed: {
	...
	methods: {
		...
		async createConnection() {
			...
			this.creating = true;
			try {
				const response = await this.$http.post(createCodexConnection, { syncIntervalHours: 24, includeInDiary: true, aiAllowed: true });
...
.command-row { display: flex; align-items: center; justify-content: space-between; gap: 14rpx; }
...
.command-row > text { -webkit-user-select: text; user-select: text; }
