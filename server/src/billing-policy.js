...
const LEGAL_VERSION = '2026-09-15-1';
...
function commercialRollout(mode, h5Live = false) {
  ...
  const copy = {
    ...
    PARTIAL_LIVE: {
      title: 'H5 充值已开放',
      description: '当前仅支持微信外的系统浏览器；小程序和原生 APP 仍分别等待虚拟支付与应用商店内购接入。'
    }
  }[phase];
  return {
    phase,
...
function legalDocument(key, merchant = {}) {
  ...
  const sections = {
    ...
    recharge: [
      ...
      ['支付渠道', 'H5 充值仅在系统浏览器内通过已审核的微信 H5 支付完成。微信小程序内的菇点属于虚拟服务，在微信虚拟支付能力通过审核并完成独立接入前，不使用普通小程序支付，也不引导用户到外部付款。'],
