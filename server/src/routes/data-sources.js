...
router.get('/activities/:id', asyncRoute(async (req, res) => {
  if (!/^[0-9a-f-]{36}$/i.test(String(req.params.id || ''))) return fail(res, 400, '外部活动标识不正确');
  const activity = await getDiarySourceActivity(db, req.user.id, req.params.id);
  if (!activity) return fail(res, 404, '这条外部活动不存在');
