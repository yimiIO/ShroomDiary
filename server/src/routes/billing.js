...
router.post('/payments/wechat/notify', async (req, res) => {
  const v2Jsapi = config.billing.wechatPay.protocol === 'v2_jsapi';
  try {
    if (!paymentConfiguration().callbackReady) return res.status(503).json({ code: 'FAIL', message: '支付回调未就绪' });
    const transaction = parseNotification(req.headers, req.rawBody || req.body);
    ...
    if (v2Jsapi) return res.type('application/xml').send(v2NotificationResponse(true));
    return res.status(204).end();
  } catch (error) {
    console.error('wechat payment notification rejected', { code: error.code, message: error.message });
    if (v2Jsapi) return res.type('application/xml').send(v2NotificationResponse(false, '回调验证失败'));
...
function safeWalletReturnUrl(value) {
  const origin = new URL(config.publicOrigin);
  const walletUrl = `${origin.origin}/pages/shroom/wallet`;
  try {
    const target = new URL(String(value || ''), origin);
    if (target.origin !== origin.origin || target.pathname !== '/pages/shroom/wallet') return walletUrl;
    target.hash = '';
...
router.get('/payments/wechat-jsapi/auth-url', (req, res) => {
  const channel = paymentChannelConfiguration('H5');
  if (!channel.live || channel.paymentMode !== 'JSAPI') {
    return fail(res, 503, channel.reason || '微信内网页支付尚未开放');
  }
  if (!isPersonalWechatBrowser(req.get('user-agent'))) return fail(res, 400, '请在个人微信内打开菇日记');
  const returnUrl = safeWalletReturnUrl(req.query.returnUrl);
  const state = createOAuthState({ userId: req.user.id, returnUrl });
  return ok(res, { authUrl: oauthAuthorizationUrl({ state, redirectUrl: returnUrl }) });
});

router.post('/payments/wechat-jsapi/payer', asyncRoute(async (req, res) => {
  const channel = paymentChannelConfiguration('H5');
  if (!channel.live || channel.paymentMode !== 'JSAPI') {
    return fail(res, 503, channel.reason || '微信内网页支付尚未开放');
  }
  if (!isPersonalWechatBrowser(req.get('user-agent'))) return fail(res, 400, '请在个人微信内完成支付授权');
  const state = verifyOAuthState(req.body.state, { userId: req.user.id });
  const oauth = await exchangeOAuthCode(text(req.body.code, 256));
  return ok(res, {
    payerToken: createPayerToken({ userId: req.user.id, openid: oauth.openid }),
...
router.post('/payments/orders', asyncRoute(async (req, res) => {
  ...
  try {
    const created = await createPayment(order, {
      ...
      userId: req.user.id,
      payerToken: text(req.body.payerToken, 2048)
    });
