...
const FOLLOWUP_PROMPT = `你是「日记行动、菇卡、未解之问与复利方向关联审阅器」。根据日记、当前启用观察席的结果、用户已有菇卡、已有未解之问和复利系统长期方向，同时完成待办提取、菇卡判断、未解之问候选识别与复利方向关联。
