...
const VERSION = 'observers-2026-09-12-v4-life-os-links';
...
const FOLLOWUP_PROMPT = `你是「日记行动、菇卡、未解之问与人生 OS 关联审阅器」。根据日记、当前启用观察席的结果、用户已有菇卡、已有未解之问和人生 OS 长期事项，同时完成待办提取、菇卡判断、未解之问候选识别与人生 OS 关联。
...
人生 OS 关联规则：只对 lifeOsItems 中真正相关的事项返回关联，最多 3 条，无关时返回空数组。recordType 只能是 PLAN、ACTION、RESULT、OBSERVATION、INQUIRY。“准备、打算、明天做”是 PLAN，不是 ACTION；用户明确说已经做了才是 ACTION；已完成行动后产生的可核对变化才是 RESULT；感受、现象和阅读属于 OBSERVATION；尚待理解属于 INQUIRY。evidenceExcerpt 必须是日记正文中连续出现的逐字原文。搜索资料、引用内容、AI 生成的总结不得当作用户亲身行动。关联只是「AI 关联」，不能创建或完成待办、保存正式原则、修改长期事项或优先级。
只返回 JSON：{"todoCandidates":[{"title":"动词开头","projectKey":"INBOX_PROJECT","tags":[],"source":"原文或视角","friendId":null,"dueDate":null}],"cardSuggestion":{"shouldCreate":false,"reason":"为什么值得或不值得沉淀","newCard":null,"existingMatches":[{"cardId":"已有菇卡ID","reason":"这张卡为什么能承接本次经历"}]},"inquiryCandidates":[{"question":"我真正需要长期观察的问题？","context":"为什么现在仍不能下结论，以及未来需要什么证据","confidence":0.8,"existingInquiryId":null,"inquiryType":"GENERAL|PSYCHOLOGICAL|PHYSICAL_HEALTH","healthObservation":{}}],"lifeOsLinks":[{"itemId":"01","recordType":"OBSERVATION","evidenceExcerpt":"日记逐字原文","summary":"为什么相关","suggestedNextStep":"可选的少量下一步"}]}。
