...
const VERSION = 'observers-2026-09-13-v6-structured-health';
...
身心记录规则：它是独立的候选事实层，不以用户是否提出问题为前提，并且必须在本次 followup 请求中一次完成，禁止为心理、身体或生活因素分别再调用模型。只有日记明确写到心理感受、压力与认知变化、身体症状、睡眠、生活行为与环境、测量或检查结果时才返回 healthExtraction；否则各数组为空。每个候选的 evidenceExcerpt 必须是日记正文中连续出现的逐字原文。“可能、也许、好像、怀疑、不确定”等表达的 certainty 必须是 UNCERTAIN，不能自动转成事实。只记录观察，不推断疾病、人格或因果。healthInquiryLinks 只能引用 existingInquiries 中真实存在的心理/身体问题 id，不得自动建立关联。missingInformation 只写日记未说清、后续确认有价值的信息。redFlags 只在原文明确包含需立即或尽快求助的危险信号时返回，不得根据模糊表达扩大判断。
未解之问规则：它只组织需要长期回答的问题，不承载原始身心记录。它必须是当前日记和一次分析无法可靠回答、需要未来经历、行为结果、反例或跨时间比较才能逐步理解的个人问题。它可以来自正文中的疑问，也可以来自反复模式的陈述，不依赖问号、标签或情绪选择。不要把临时不知道的事实、可以立即搜索的问题、修辞性抱怨、普通待办、一次性情绪、已经形成答案的观点、医学诊断或泛泛的人生大问题识别为候选。问题应使用第一人称、具体、中性且可被未来证据修订；没有足够依据时返回空数组，最多 2 个。若问题需要引用心理身心记录作为证据，inquiryType 使用 PSYCHOLOGICAL；若需要引用身体身心记录，使用 PHYSICAL_HEALTH；其他使用 GENERAL。若与 existingInquiries 中同类型的问题本质相同，填写其真实 id 到 existingInquiryId，不要换句话重复创建。confidence 是 0 到 1 的识别把握；只有至少 0.65 才输出。context 简要说明为什么现在仍不能回答、未来需要什么证据，不得虚构。
