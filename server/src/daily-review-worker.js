...
function renderReviewText(review) {
  ...
  if (value.ownershipDecisions?.length) {
    sections.push('', '哪些不该自己做', ...value.ownershipDecisions.map(item =>
  ...
  sections.push('', '今日事实（仅作依据）', value.headline || '', value.factsSummary || '暂无足够记录。');
...
function renderReviewHtml(review) {
  ...
  const criticalBody = `<div style="padding:20px;border-radius:16px;background:#f2e2dc"><strong style="font-size:20px;color:#6f3028">${escapeHtml(critical.issue)}</strong><p style="line-height:1.75;margin:12px 0 0">${escapeHtml(critical.consequence || '')}</p><div style="margin-top:16px;padding-top:14px;border-top:1px solid #dfc3b9"><span style="font-size:12px;color:#8e5c52">建议｜明天只改这一件</span><p style="font-weight:700;line-height:1.7;margin:6px 0 0">${escapeHtml(critical.recommendation || '')}</p></div></div>`;
  return `<!doctype html><html><body style="margin:0;background:#f1f8e9;color:#172019;font-family:-apple-system,BlinkMacSystemFont,'PingFang SC',sans-serif"><main style="max-width:640px;margin:0 auto;padding:34px 24px 48px"><div style="font-size:12px;letter-spacing:2px;color:#728074">SHROOM DAILY REVIEW · ${escapeHtml(review.date)}</div><h1 style="font-family:Georgia,'Songti SC',serif;font-size:30px;line-height:1.35;margin:12px 0">先看问题，不看成绩单</h1>${section('今天最需要修正', criticalBody)}${section('哪些不该自己做', htmlList(value.ownershipDecisions, item => `<strong>${escapeHtml(OWNER_LABELS[item.owner] || item.owner)}</strong>｜${escapeHtml(item.task)}：${escapeHtml(item.reason)}`))}${section('人生 OS', htmlList(value.lifeOsAudit, item => `<strong>${escapeHtml(OS_LABELS[item.status] || item.status)}</strong>｜${escapeHtml(item.principle || '未命名原则')}：${escapeHtml(item.reason)}`))}${section('复利判断', htmlList(compoundItems, item => `<strong>${escapeHtml(item.label)}</strong>：${escapeHtml(item.text)}`))}${section('今日事实（仅作依据）', `<strong>${escapeHtml(value.headline || '')}</strong><p style="line-height:1.8">${escapeHtml(value.factsSummary || '暂无足够记录。')}</p>`)}${section('证据边界', htmlList(value.caveats, item => escapeHtml(item)))}<p style="margin-top:34px;padding-top:18px;border-top:1px solid #dce6d6;color:#718075;font-size:12px;line-height:1.7">数据截止：${escapeHtml(review.sourceCutoff ? new Date(review.sourceCutoff).toLocaleString('zh-CN', { timeZone: 'Asia/Shanghai' }) : '未知')}。这是一份可修订的复盘草稿，不会自动修改你的人生 OS、复利状态或待办。你可以在菇的每日总结设置中关闭邮件。</p></main></body></html>`;
}
