...
test('daily review UI is user-opened and labels evidence boundaries', () => {
  ...
  assert.match(page, /建议｜明天只改这一件/);
  assert.ok(page.indexOf('今天最需要修正') < page.indexOf('哪些不该继续自己做'));
  assert.ok(page.indexOf('哪些不该继续自己做') < page.indexOf('今日事实，仅作判断依据'));
...
test('daily review generation and email put correction before the activity recap', () => {
  ...
  const worker = source('server/src/daily-review-worker.js');
  assert.match(generator, /daily-review-v2-critical-first-r2/);
  assert.match(generator, /不得断言用户亲手执行/);
  assert.match(generator, /criticalReview/);
  assert.match(generator, /不要用大量完成清单占据开头/);
  assert.ok(worker.indexOf("'今天最需要修正'") < worker.indexOf("'今日事实（仅作依据）'"));
});
