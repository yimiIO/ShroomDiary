...
const inquiries = [{ id: '11111111-1111-4111-8111-111111111111', inquiryType: 'PHYSICAL_HEALTH' }];
...
test('one diary health extraction keeps grounded observations and uncertainty', () => {
  const result = normalizeDiaryHealthExtraction({
    ...
    healthInquiryLinks: [{
      inquiryId: inquiries[0].id, reason: '可能相关',
      evidenceExcerpt: '今天觉得胸口发紧', confidence: 0.8
    }],
    missingInformation: ['缺少持续时间'],
    redFlags: [{ signal: '胸口不适正在加重', evidenceExcerpt: '现在胸口发紧越来越明显', urgency: 'EMERGENCY' }]
  }, { diaryContent: diary, existingInquiries: inquiries });
  ...
  assert.equal(result.healthInquiryLinks[0].inquiryId, inquiries[0].id);
  assert.equal(result.redFlags[0].urgency, 'EMERGENCY');
  assert.match(result.redFlags[0].action, /急救|急诊/u);
});

test('ungrounded health links and red flags are discarded', () => {
  const result = normalizeDiaryHealthExtraction({
    healthInquiryLinks: [{ inquiryId: inquiries[0].id, evidenceExcerpt: '日记没有这句话', confidence: 0.9 }],
    redFlags: [{ signal: '危险', evidenceExcerpt: '日记没有这句话', urgency: 'EMERGENCY' }]
  }, { diaryContent: diary, existingInquiries: inquiries });

  assert.deepEqual(result.healthInquiryLinks, []);
...
test('diary health extraction has a stable empty shape and legacy inquiry projection', () => {
  ...
  assert.deepEqual(Object.keys(empty), [
    ...
    'environmentFactors', 'healthInquiryLinks', 'missingInformation', 'redFlags'
  ]);
  ...
  }, { diaryContent: '很焦虑，头痛，只睡四小时。', existingInquiries: [] });
