...
test('daily review does not invent criticism when the critical evidence reference is invalid', () => {
  ...
  assert.equal(result.criticalReview.consequence, '');
});

test('daily review keeps the visible judgment and action short', () => {
  const result = normalizeDailyReview({
    criticalReview: {
      issue: '问'.repeat(80), recommendation: '做'.repeat(100), sourceRefs: ['D1']
    }
  }, context());
  assert.equal(result.criticalReview.issue.length, 44);
  assert.equal(result.criticalReview.recommendation.length, 60);
});

test('daily review format version invalidates older generated summaries', () => {
  assert.equal(DAILY_REVIEW_VERSION, 'daily-review-v3-one-judgment-r1');
...
test('email rendering escapes private content and carries the single adjustment', () => {
  ...
  assert.match(plain, /明天只做这一件\n只做一件/);
  assert.doesNotMatch(plain, /今日事实/);
  assert.doesNotMatch(html, /SHROOM DAILY REVIEW/);
  assert.doesNotMatch(html, /关键结果没有推进/);
});
