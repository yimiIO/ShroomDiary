...
2. 不要输出可立即搜索的事实问题、普通待办、修辞抱怨、一次性情绪、已经有答案的观点、医学诊断或泛泛人生问题。
3. 问题使用第一人称、具体、中性、可被未来证据修订。一次日记的情绪不能直接升级为长期模式。
4. 只有证据充分时才输出；每批最多 5 个，没有就返回空数组。
5. 如果本质上属于 existingInquiries，填写其真实 id；不要杜撰 id。

只返回 JSON：{"candidates":[{"question":"我需要长期观察的具体问题？","context":"目前为什么不能下结论、未来需要观察什么","confidence":0.8,"sourceDiaryIds":["真实日记ID"],"existingInquiryId":null}]}`;
...
4. 最多 12 个，按长期价值和证据强度排序；confidence 低于 0.65 的不要输出。
5. 这只是待用户确认候选，不能宣布人格判断或替用户得出答案。
