...
6. 未解之问与身心记录是两条独立分析线：这里只判断“是否值得长期追问”，不生成或引用身心观察。
7. 只有证据充分时才输出；每批最多 5 个，没有就返回空数组。
8. 如果本质上属于 existingInquiries，填写其真实 id；类型必须一致，不要杜撰 id。

只返回 JSON：{"candidates":[{"question":"我需要长期观察的具体问题？","context":"目前为什么不能下结论、未来需要观察什么","confidence":0.8,"inquiryType":"GENERAL","sourceDiaryIds":["真实日记ID"],"existingInquiryId":null}]}`;
...
4. 保留每个候选的 inquiryType；合并普通与健康候选时以更谨慎的健康类型为准。不生成或引用身心记录。
5. 健康类只作为观察和线索，不得诊断、进行因果断言、提供药物或治疗方案。
6. 最多 12 个，按长期价值和证据强度排序；confidence 低于 0.65 的不要输出。
7. 这只是待用户确认候选，不能宣布人格判断或替用户得出答案。

只返回 JSON：{"candidates":[{"question":"我需要长期观察的具体问题？","context":"目前为什么不能下结论、未来需要观察什么","confidence":0.8,"inquiryType":"PSYCHOLOGICAL","sourceDiaryIds":["真实日记ID"],"existingInquiryId":null}]}`;
